...
get_stage_prompt() {
    ...
    case "$stage_name" in
        ...
        "execute") echo "/plan execute all remaining tasks in the plan, then verify that everything has been completed correctly in accordance with the plan. if there is nothing left to do (within the plan and scope), then just return (do nothing). if there are remaining items, lets keep going. This is execute iteration $iteration. $PLAN_SUFFIX" ;;
